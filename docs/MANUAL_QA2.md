...
- [X] **Point tool commits its snap**: with the Point tool (P), hover an
...
- [X] **Ribbon never wraps** (#1): shrink the window — the ribbon stays one
...
- [X] **Light-mode file dialog** (#2): Modernist Light ▸ File ▸ Open: the
...
- [X] **Sketch opens where the model was** (#3): orbit/zoom the model so a
...
- [X] **Camera saved with the file** (#4): frame an odd view, Save, Views ▸
